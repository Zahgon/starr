...
impl Lidarr {
    /// Returns the Lidarr Queue (processing, but not yet imported).
    ///
    /// If you need control over the page, use [`Lidarr::get_queue_page`].
    ...
    /// Returns a single page from the Lidarr Queue.
    ///
    ...
    pub async fn get_queue_page(&self, params: Option<&PageReq>) -> Result<Queue> {
        ...
        params.check_set("includeUnknownArtistItems", "true");

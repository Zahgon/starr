...
impl Prowlarr {
    ...
    pub async fn add_custom_filter(&self, filter: &CustomFilter) -> Result<CustomFilter> {
        self.api
            .post_into(Request::new(BP_CUSTOM_FILTER).with_json(filter)?)
            .await
    }

    /// Updates a custom filter.
    pub async fn update_custom_filter(&self, filter: &CustomFilter) -> Result<CustomFilter> {
        let req =
            Request::new(path_join(&[BP_CUSTOM_FILTER, &str_val(filter.id)])).with_json(filter)?;

...
impl Radarr {
    /// Returns the Radarr Queue (processing, but not yet imported).
    ///
    /// If you need control over the page, use [`Radarr::get_queue_page`].
    ...
    /// Returns a single page from the Radarr Queue.
    ///
    ...
    pub async fn get_queue_page(&self, params: Option<&PageReq>) -> Result<Queue> {
        ...
        params.check_set("includeUnknownMovieItems", "true");
